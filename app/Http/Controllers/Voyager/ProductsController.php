...
class ProductsController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->stock !=0){
            ...
            $details = [
                'subject'=>"$request->name disponible",
                'body'=>"Hello cher client le produit $request->name est desormais disponible dans notre boutique pour une quantité de $request->stock. Merci de vite l'acheter avant sa rupture.",
            ];



            //  Mail::to('khadidia8@gmail.com')->send(new MyMailer($details));


            $users = User::all()->where('is_newsletter', 1)->first();


            foreach ($users as $user) {
                Mail::to($user)->send(new MyMailer($details));
            }
        }
